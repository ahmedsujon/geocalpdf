...
class SoilAggregateComponent extends Component
{
    ...
    {
        $data = SoilAggregate::find($this->delete_id);
        $data->delete();
        $this->delete_id = '';
        $this->dispatchBrowserEvent('FileDeleted');
    }

    public function getSoilAggregateRepresentative(Request $request)
    {
        $output = '';
    ...
    public function editSoilAggregateRepresentative(Request $request)
    {
        $output = '';
        ...
        $responsible_persons = SoilAggregate::find($file_id)->responsible_person;

        if ($value == 'sentToClient') {
            $getCustomer = SoilAggregate::where('id', $file_id)->first()->client_id;
    ...
    public function render()
    {
        ...
        return view('livewire.soil-aggregate.soil-aggregate-component', ['files'=>$files])->layout('livewire.layouts.base');
    }
}
