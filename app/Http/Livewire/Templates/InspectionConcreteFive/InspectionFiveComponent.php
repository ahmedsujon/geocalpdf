...
class InspectionFiveComponent extends Component
{
    use WithPagination;
    ...
    {
        $data = InspectionConcreteSetFive::find($this->delete_id);
        $data->delete();
        $this->delete_id = '';
        $this->dispatchBrowserEvent('FileDeleted');
    }

    public function getDataSetFiveRepresentative(Request $request)
    {
        $output = '';
    ...
    public function editDataSetFiveRepresentative(Request $request)
    {
        $output = '';
        ...
        $responsible_persons = InspectionConcreteSetFive::find($file_id)->responsible_person;

        if ($value == 'sentToClient') {
            $getCustomer = InspectionConcreteSetFive::where('id', $file_id)->first()->client_id;
    ...
    public function render()
    {
        if (Auth::user()->role_id == '1' || Auth::user()->role_id == '2') {
            ...
            $files = collect([]);
            $all_files = InspectionConcreteSetFive::orderBy('id', 'DESC')
                ->join('projects', 'inspection_concrete_set_fives.project_id', '=', 'projects.id')
                ->where('projects.name', 'like', '%' . $this->searchTerm . '%')
                ->select('inspection_concrete_set_fives.*')
                ->get();
