...
class InspectionSevenComponent extends Component
{
    use WithPagination;
    ...
    {
        $data = InspectionConcreteSetSeven::find($this->delete_id);
        $data->delete();
        $this->delete_id = '';
        $this->dispatchBrowserEvent('FileDeleted');
    }

    public function getDataSetSevenRepresentative(Request $request)
    {
        $output = '';
    ...
    public function editDataSetSevenRepresentative(Request $request)
    {
        $output = '';
        ...
        $responsible_persons = InspectionConcreteSetSeven::find($file_id)->responsible_person;

        if ($value == 'sentToClient') {
            $getCustomer = InspectionConcreteSetSeven::where('id', $file_id)->first()->client_id;
    ...
    public function render()
    {
        if (Auth::user()->role_id == '1' || Auth::user()->role_id == '2') {
            ...
            $files = collect([]);
            $all_files = InspectionConcreteSetSeven::orderBy('id', 'DESC')
                ->join('projects', 'inspection_concrete_set_sevens.project_id', '=', 'projects.id')
                ->where('projects.name', 'like', '%' . $this->searchTerm . '%')
                ->select('inspection_concrete_set_sevens.*')
                ->get();
