...
class InspectionNineComponent extends Component
{
    ...
    {
        $data = InspectionConcreteSetNine::find($this->delete_id);
        $data->delete();
        $this->delete_id = '';
        $this->dispatchBrowserEvent('FileDeleted');
    }

    public function getDataSetNineRepresentative(Request $request)
    {
        $output = '';
    ...
    public function editDataSetNineRepresentative(Request $request)
    {
        $output = '';
        ...
        $responsible_persons = InspectionConcreteSetNine::find($file_id)->responsible_person;

        if ($value == 'sentToClient') {
            $getCustomer = InspectionConcreteSetNine::where('id', $file_id)->first()->client_id;
    ...
    {
        $files = InspectionConcreteSetNine::orderBy('id', 'DESC')->paginate($this->sortingValue);
