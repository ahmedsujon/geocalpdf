...
class FilesComponent extends Component
{
    ...
    public function editRepresentative(Request $request)
    {
        ...
        $responsible_persons = File::find($file_id)->responsible_person;

        if ($value == 'sentToClient') {
            $getCustomer = File::where('id', $file_id)->first()->client_id;
    ...
    {
        $files = File::orderBy('id', 'DESC')->paginate($this->sortingValue);
        return view('livewire.files.files-component', ['files' => $files])->layout('livewire.layouts.base');
    }
}
